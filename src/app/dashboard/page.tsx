...
export default function Dashboard() {
	...
	const fetchCustomers = useCallback(async () => {
		try {
			...
			if (data.customers.length > 0) {
				setCustomer(data.customers[0].name);
			}
		} catch (err) {
			console.log(err);
		}
	...
	// fetch bank info and customers on page load
	useEffect(() => {
		if (user) {
			fetchBankInfo();
			if (bankInfoExists) {
				...
				{!bankInfoExists ? (
					<div className='md:w-5/6 w-full h-screen flex-col p-6 flex items-center justify-center'>
						<p className='text-lg font-bold mb-3'>
							Welcome! Please register your banking information to start creating invoices.
						</p>
						<Link
							href='/settings'
						...
						<form className='w-full flex flex-col' onSubmit={handleFormSubmit}>
							...
							{customers.length > 0 ? (
								...
								<p className='text-sm text-red-500'>
									No customers found. Please add a customer using the 'Customers' tab first!
								</p>
							)}
